...
{
    fun toUserData() = UserData(name, email, Base64.getDecoder().decode(avatar), streak, longestStreak, completedTasks, wonDays, daysIn, allTasks.map { toTaskData(it) }.toMutableList(), preparedTasks.map { toTaskData(it) }.toMutableList(), activeTasks.map { toTaskData(it) }.toMutableList())
